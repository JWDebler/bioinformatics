...
// For this script to work and name everything correctly you need to concatenate all those files into one .fastq.gz
...
process Canu_ont {
    ...
    input:
    tuple sampleID, 'input.nanopore.fastq.gz' from nanoporeReadsForAssembly
...
process racon {
    ...
    input:
    tuple sampleID, 'input.fasta', 'input.nanopore.fastq.gz' from racon

    output:
    tuple sampleID, "${sampleID}.contigs.racon.fasta", 'input.nanopore.fastq.gz' into medaka
...
process medaka {
    ...
    input:
    tuple sampleID, 'input.fasta', 'input.nanopore.fastq.gz' from medaka

    output:
    tuple sampleID, "${sampleID}.contigs.racon.medaka.fasta", 'input.nanopore.fastq.gz' into pilon
